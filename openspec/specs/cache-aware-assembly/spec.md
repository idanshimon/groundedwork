# cache-aware-assembly Specification

## Purpose
Cache-friendly prompt assembly via `messages()`. The stable `[system + prefs]` content is placed first and the volatile retrieved knowledge last, so a prompt-caching endpoint (Anthropic/OpenAI/Azure) reuses the prefix across queries. The load-bearing rule: retrieved knowledge is never placed in the system block, so per-query injection does not invalidate the cached prefix — the opposite of burying retrieved content in the system message.
## Requirements
